...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        // Rasm: local fayl yoki URL (eski qiymat saqlanadi)
        $curImg = '';
        if ($id) {
            $g = db()->prepare('SELECT image FROM products WHERE id=? AND store_id=?');
            $g->execute([$id, $sid]); $curImg = (string)($g->fetchColumn() ?: '');
        }
        $upErr = null;
